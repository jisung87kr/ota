...
@section('title', '숙소 정보 수정 - OTA Service')

@section('content')
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">숙소 정보 수정</h1>
        <p class="mt-2 text-gray-600">{{ $accommodation->name }}의 정보를 수정하세요</p>
    </div>

    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('manager.accommodations.update', $accommodation->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <select name="category" id="category" required
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat }}" {{ old('category', $accommodation->category) === $cat ? 'selected' : '' }}>
                                    {{ ucfirst($cat) }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="5" required
                                  class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $accommodation->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    @if($accommodation->main_image)
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">현재 대표 이미지</label>
                    ...
                    <div>
                        <label for="main_image" class="block text-sm font-medium text-gray-700 mb-2">
                            대표 이미지 변경 <span class="text-gray-500">(최대 2MB, 변경 시에만 선택)</span>
                        </label>
                    ...
                    <div>
                        <label class="flex items-center cursor-pointer">
                            ...
                            <span class="ml-2 text-sm font-medium text-gray-700">활성 상태 (체크 해제 시 검색 결과에 표시되지 않습니다)</span>
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-8 border-t pt-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="tel" name="phone" id="phone" value="{{ old('phone', $accommodation->phone) }}" required
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $accommodation->email) }}"
